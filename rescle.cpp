...
std::vector<BYTE> VersionInfo::Serialize() const {
	...
	if (HasFixedFileInfo()) {
		...
		versionInfo.valueLength = (WORD)size; // caa0031, C4267 warning
...

	{
		...
		for (const auto& iTable : stringTables) {
			...
			for (const auto& iString : iTable.strings) {
				...
				stringRaw.valueLength = (WORD)strLenNullTerminated; // caa0031, C4267 warning
...

	{
...

		{
...

			{
				...
				for (auto iVar = 0; iVar < (int)supportedTranslations.size(); ++iVar) { // caa0031, C4018 on x86 
				...
				varRaw.valueLength = (WORD)varRaw.value.size(); // caa0031, C4267 warning
			}
...
VersionStringTable VersionInfo::DeserializeVersionStringTable(const BYTE* tableData) {
	...
	tableEntry.encoding.wCodePage = (WORD)langIdCodePagePair; // caa0031, C4244 warning
...
void VersionInfo::DeserializeVarFileInfo(const unsigned char* offset, std::vector<Translate>& translations) {
	...
	for (auto pTranslatePair = top; pTranslatePair < top + translatePairs.second; pTranslatePair += sizeof(DWORD)) {
		...
		Translate translate;
		translate.wLanguage = (LANGID)codePageLangIdPair; // caa0031, C4244 warning
...
bool ResourceUpdater::Load(const WCHAR* filename) {
	...
	EnumResourceNamesW(module_, RT_STRING, (ENUMRESNAMEPROCW)OnEnumResourceName, reinterpret_cast<LONG_PTR>(this));
	EnumResourceNamesW(module_, RT_VERSION, (ENUMRESNAMEPROCW)OnEnumResourceName, reinterpret_cast<LONG_PTR>(this));
	EnumResourceNamesW(module_, RT_GROUP_ICON, (ENUMRESNAMEPROCW)OnEnumResourceName, reinterpret_cast<LONG_PTR>(this));
	EnumResourceNamesW(module_, RT_ICON, (ENUMRESNAMEPROCW)OnEnumResourceName, reinterpret_cast<LONG_PTR>(this));
	EnumResourceNamesW(module_, RT_MANIFEST, (ENUMRESNAMEPROCW)OnEnumResourceManifest, reinterpret_cast<LONG_PTR>(this));
	EnumResourceNamesW(module_, RT_RCDATA, (ENUMRESNAMEPROCW)OnEnumResourceName, reinterpret_cast<LONG_PTR>(this));
...
bool ResourceUpdater::ChangeString(WORD languageId, UINT id, const WCHAR* value) {
	...
	if (table.find(blockId) == table.end()) {
		...
		for (size_t i = table.size(); i <= blockId; ++i) {
			table[(UINT)i] = std::vector<std::wstring>(16); // caa0031, warning C4267
		}
	}
...
bool ResourceUpdater::SetIcon(const WCHAR* path, const LANGID& langId,
	...
	for (size_t i = 0; i < header.count; ++i) {
		...
		if (!ReadFile(file, icon.images[i].data(), (DWORD)icon.images[i].size(), &bytes, NULL)) { // caa0031, C4267 warning
	...
	for (size_t i = 0; i < header.count; ++i) {
		...
		entry->bytesInRes2 = (WORD)header.entries[i].bytesInRes; // caa0031, C4244 warning
		entry->colourCount = header.entries[i].colorCount;
		entry->height = header.entries[i].height;
		entry->id = (WORD)i + 1; // caa0031, C4267 warning
		entry->planes = (BYTE)header.entries[i].planes; // caa0031, C4244 warning
...
bool ResourceUpdater::Commit() {
	...
	if (applicationManifestPath_.empty() && !executionLevel_.empty()) {
		...
		if (!UpdateResourceW(ru.Get(), RT_MANIFEST, MAKEINTRESOURCEW(1),
			//kLangEnUs, // this is hardcoded at 1033, ie, en-us, as that is what RT_MANIFEST default uses // andreiaugustin - we need to use kLangEnGb (2057).
			kLangEnGb, // andreiaugustin
			&stringSection.at(0), sizeof(char) * (DWORD)stringSection.size())) { // caa0031, C4267 warning
			return false;
		}
	}
	...
	for (const auto& rcDataLangPair : rcDataLngMap_) {
		for (const auto& rcDataMap : rcDataLangPair.second) {
			if (!UpdateResourceW(ru.Get(), RT_RCDATA, reinterpret_cast<LPWSTR>(rcDataMap.first),
				rcDataLangPair.first, (LPVOID)rcDataMap.second.data(), (DWORD)rcDataMap.second.size())) { // caa0031, C4267 warning
				return false;
			}
		}
	}
		...
		for (const auto& iNameBundlePair : iLangIconInfoPair.second.iconBundles) {
			...
			if (icon.grpHeader.size() > 0) {
				if (!UpdateResourceW(ru.Get(), RT_GROUP_ICON, MAKEINTRESOURCEW(bundleId),
					langId, icon.grpHeader.data(), (DWORD)icon.grpHeader.size())) { // caa0031, C4267 warning
					return false;
				}

				for (size_t i = 0; i < icon.header.count; ++i) {
					if (!UpdateResourceW(ru.Get(), RT_ICON, MAKEINTRESOURCEW(i + 1),
						langId, icon.images[i].data(), (DWORD)icon.images[i].size())) { // caa0031, C4267 warning
	...
	if (IS_INTRESOURCE(lpszName) && IS_INTRESOURCE(lpszType)) {
		switch (reinterpret_cast<ptrdiff_t>(lpszType)) {
			case reinterpret_cast<ptrdiff_t>(RT_VERSION) : {
				...
				catch (const std::system_error) { // caa0031, C4101 warning for error var being received but never used, may be useful to keep in when debugging
					return false;
				}
				break;
			}
			case reinterpret_cast<ptrdiff_t>(RT_STRING) : {
				UINT id = (UINT)reinterpret_cast<ptrdiff_t>(lpszName) - 1; // caa0031 - C4244 warning
				auto& vector = instance->stringTableMap_[wIDLanguage][id];
				for (size_t k = 0; k < 16; k++) {
					CStringW buf;

					buf.LoadStringW(instance->module_, id * 16 + (UINT)k, wIDLanguage); // caa0031 - C4267 warning 
					vector.push_back(buf.GetBuffer());
				}
				break;
			}
			case reinterpret_cast<ptrdiff_t>(RT_ICON) : {
				UINT iconId = (UINT)reinterpret_cast<ptrdiff_t>(lpszName); // caa0031 - C4244 warning
...
BOOL CALLBACK ResourceUpdater::OnEnumResourceManifest(HMODULE hModule, LPCTSTR lpType, LPWSTR lpName, LONG_PTR lParam) {
	...
	int len = (int)strlen(reinterpret_cast<const char*>(pResource)); // caa0031 - C4267 warning
